...
- [Gateway Latency Exceeded](#gateway-latency-exceeded)
...
### Gateway Latency Exceeded

**Trigger Severity:** ERROR  
**Resolution Severity:** INFO

**Description:**
- **Trigger:** Alerts when measured latency to a gateway exceeds the configured threshold of a [gateway latency monitor]({{<relref "/docs/nodes/appliances/gateway/gateway-client#gateway-latency-monitors">}}). The event details include the destination gateway name.
- **Resolution:** Alerts when latency to the gateway returns below the threshold.
...
**Trigger Message:** Cluster is offline
**Resolution Message:** Cluster connectivity restored

**Description:**
- **Trigger:** Alerts when both cluster members are down .
- **Resolution:** Alerts when a cluster re-establishes connectivty.
...
**Description:** Alerts when a [node's public IP has been locked]({{<ref "/tutorials/management-tasks/limit-node-functionality" >}}) but the connection to the control plane comes from a different IP.
